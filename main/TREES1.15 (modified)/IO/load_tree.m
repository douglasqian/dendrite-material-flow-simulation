...
% - options::string {DEFAULT : '-r -a' for .swc/.neu '' for .mtr}
%     '-s' : show
...
switch format,
    ...
    case '.swc' % this is then swc
        if ~exist ([path tname], 'file'),
            error ('.swc file nonexistent...');
        end
        % Creates a nrows * 1 cell array representing the swc file
        A = textread ([path tname], '%s', 'delimiter', '\n');
        swc = [];
        cur_id = 1;
        
        new_vals = containers.Map(1, 1, 'UniformValues', 1);
        
        for ward = 1 : length (A),
            if ~isempty (A {ward}), % don't allow empty lines in between
                if ~strcmp (A {ward} (1), '#') % don't allow comments: lines starting with #
                    line = str2num(A{ward});
                    id = line(1);
                    swc_type = line(2);
                    pid = line(7);
                    if swc_type ~= 2 % don't allow axon points
                        if swc_type == 1 && id ~= 1 % only allow 1st soma pt (no 3 pt somas)
                            new_vals(id) = pid;
                        else
                            if id == 1
                                swc = [swc; line];             
                            else
                                % Store information about changes in new_vals
                                new_vals(id) = cur_id;
                                line(1) = cur_id;
                                % Parent id look ups
                                line(7) = new_vals(pid);
